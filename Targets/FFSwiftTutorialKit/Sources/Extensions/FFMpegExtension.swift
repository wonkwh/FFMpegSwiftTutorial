...
public func toDictionary(_ native: OpaquePointer?) -> [String: String] {
...
public extension UnsafeMutablePointer where Pointee == AVCodecContext {
  func getFormat() {
...
extension AVChannelLayout: CustomStringConvertible {
  ...
  var layoutTag: AudioChannelLayoutTag? {
    debugPrint("[audio] FFmpeg AVChannelLayout: \(self) order: \(order) mask: \(u.mask)")
    ...
    if let tag {
      ...
      debugPrint("[audio] can not find AudioChannelLayoutTag FFmepg channelLayout: \(self) order: \(order) mask: \(u.mask)")
      return nil
    }
  }
