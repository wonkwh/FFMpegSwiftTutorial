...
extension AudioUnit {
  var channelLayout: UnsafeMutablePointer<AudioChannelLayout> {
    ...
    debugPrint("[audio] unit tag: \(tag)")
    if tag == kAudioChannelLayoutTag_UseChannelDescriptions {
      debugPrint("[audio] unit channelDescriptions: \(layout.channelDescriptions)")
      return layout
    }
...
extension AudioChannelLayoutTag {
  var channelLayout: UnsafeMutablePointer<AudioChannelLayout> {
    ...
    let newLayout = data.bindMemory(
      to: AudioChannelLayout.self,
      capacity: 1
    )
...
public extension AudioChannelBitmap {
  var channelLayout: UnsafeMutablePointer<AudioChannelLayout> {
    ...
    AudioFormatGetPropertyInfo(
      ...
      UInt32( MemoryLayout<AudioChannelBitmap>.size ),
      &mChannelBitmap,
      &size
    )
...
public extension UnsafePointer<AudioChannelLayout> {
  var channelDescriptions: [AudioChannelDescription] {
    UnsafeMutablePointer(mutating: self).channelDescriptions
  }
}

public extension UnsafeMutablePointer<AudioChannelLayout> {
  var channelDescriptions: [AudioChannelDescription] {
...
extension AVAudioChannelLayout {
  func channelLayout() -> AVChannelLayout {
    debugPrint("[audio] channelLayout: \(layout.pointee.description)")
    var mask: UInt64?
    if layoutTag == kAudioChannelLayoutTag_UseChannelDescriptions {
      ...
      for description in layout.channelDescriptions {
        ...
        debugPrint("[audio] label: \(label)")
        let channel = label.avChannel.rawValue
        debugPrint("[audio] avChannel: \(channel)")
        if channel >= 0 {
          newMask |= 1 << channel
        }
      }
    ...
    debugPrint("[audio] out mask: \(outChannel.u.mask) nb_channels: \(outChannel.nb_channels)")
    return outChannel
  }
